...
inputName.addEventListener("input", e =>{
  ...
  if(value.trim().length == 0){
    ...
    span.classList.remove("check", "error"); //글씨 색상
    checkList["inputName"] = false; // 콘솔 checkList에서 확인
...
inputPw.addEventListener("input", e => {
  ...
  if (value.length === 0) { // 입력된 값이 없을 경우
      ...
      checkPwFn(); // 비밀번호, 비밓번호확인 값 일치 여부 확인 함수 호출
