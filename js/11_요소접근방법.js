...
function classTest(){
    ...
    /* 유사 배열이란?
      베열처럼 index, length를 가지고 있으나
      배열 전용 기능 (베서드)를 제공하지 않음
    */
...
function nameTest(){
    ...
    for(let i =  0; i<hobbyList.length; i++){ // 순차 접근
      ...
      // input요소. checked = flase -> 해당 요소를 체크 해제
      ...
      // 체크된 요소의 값을 str 변수에 누적 + count 변수 값을 1 증가
...
/* 아이디가 user-input인 요소에서
  ...
  올라온 키가 "Enter" 키 이면 reacValue() 함수를 호출
*/
