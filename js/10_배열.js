...
function check3(){
    ...
    // -> for 문을 이용햇 배열의 모든 요소 접근하기
    // -> 순차접근 또는 반복 접근

    const arr = [10,20,50,100,500,1000];

    // index는 0부터 배열 길이-1 까지 1씩 증가
    // i <= arr.length - 1 == i < arr.lenght
 ...
 */
 function orderFn(){
    ...
    // 메뉴 배열
    const menus = ["라면", "김밥", "샌드위치", "우동", "돈까스"];
    // 가격 배열
    const prices = [4000, 3000, 2000, 5500, 6000];

    // 주문한 메뉴 카운트 배열(어떤메뉴를 몇개 주문했는가)
    ...
    while(true){ // 무한 반복
        ...
        //     // menus[i] 값이 같다면 idx에 i 값을 저장하겠다
        ...
        if(input == null){ // 수량 입력 취소 -> 다시 메뉴입력
            continue;
        ...
        // idx : 선택한 메뉴가 존재하는 index번호
        // input : 입력한 수량 (numer 자료형)
