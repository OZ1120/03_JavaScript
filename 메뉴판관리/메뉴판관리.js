...
/* 요소 생성 속성추가 클래스 추가 함수 */
const newEl= (tag, attr, cls) => {
  const el =document.createElement(tag); /* 요소생성 */

  for(let key in attr){
    el.setAttribute(key.attr[key]); /* 객체로 전달받은 속성을 요소에 추가 */
  }
...
const creatMenuCount = ()=> {
  ...
  // 메뉴 가격 input 생성
  const menuPriceInput = newEl("input", {type:"text", placeholder:"가격"}, ["menu-name-input"]);
...
/* input 태그가 focus를 잃었을 때 (bulr) */
resNameInput.addEventListener("blur", e=>{
  resName.textContent = e.target.value;

  if(e.target.value.trim().length == 0) resName.texContent="식당명을 입력하세요";
  e.target.classList.toggl("res-name-hidden");
